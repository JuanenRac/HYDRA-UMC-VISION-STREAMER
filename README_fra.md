...
* 📷 **Réel v0 - capture+diffusion de caméras USB et IP par RTSP :** `stream serve` de `mjpeg_server.py` ouvre réellement une caméra et diffuse du vrai MJPEG par HTTP - un périphérique USB/V4L2 (`--device 0`) ou une vraie caméra IP par RTSP (`--device rtsp://utilisateur:motdepasse@host:port/chemin`, via le backend `cv2.CAP_FFMPEG` d'OpenCV, sans dépendance supplémentaire). Le `CameraConfig` de `config.py` porte désormais un vrai `source_type` (`"usb"` par défaut, ou `"ip"` avec `host`/`rtsp_port`/`rtsp_path`/`username`/`password` et un constructeur `rtsp_url()` réel avec percent-encoding). Vérifié de bout en bout sur du matériel réel : les 4 vraies caméras IP du réseau local se sont ouvertes et ont diffusé de vraies trames MJPEG/H.264 par ce même chemin (la 2ème paire avait besoin de son propre chemin RTSP réel, `profile0`, trouvé sur l'écran de config de la caméra elle-même - un chemin différent du `/11` de la première paire, pas un problème d'identifiants ou de firmware).
* 🔁 **Réel v0 - mise en tampon bornée et reconnexion :** `FrameBuffer` de `buffer.py` est une file à capacité fixe qui rejette l'élément le PLUS ANCIEN (jamais le plus récent) une fois pleine - la vraie politique de contre-pression dont un relais en direct a besoin pour qu'un consommateur lent ne puisse jamais faire croître indéfiniment la mémoire de ce processus. `ConnectionTracker` de `reconnect.py` est une vraie politique de reconnexion déterministe à backoff exponentiel pour un lien caméra/relais tombé. Le vrai chemin de capture de `mjpeg_server.py` met en tampon un `Frame` (pas seulement des octets bruts) - une vraie identité `frame_id`/`session_id`/`capture_ts`, avec un `session_id` réellement nouveau à chaque reconnexion réelle, afin que deux trames d'avant/après un lien tombé ne soient jamais confondues comme un flux continu unique. Exposé via `stream simulate` ci-dessous - entièrement testable sans GStreamer ni caméra physique.
...
│       ├── mjpeg_server.py     # Vrai serveur MJPEG - sert réellement l'image d'une webcam USB ou d'une caméra IP par RTSP via HTTP
...
5. **Suite de tests réelle** - `python -m pytest tests/ -q` (83 tests couvrant config, pipeline, génération MediaMTX, la politique de tampon/reconnexion, la frontière d'intégration HailoRT, et le CLI).
...
Exemple réel - capture+diffuse réellement une caméra en MJPEG par HTTP (le seul sous-commande qui ouvre un vrai périphérique) :

```bash
# Périphérique USB/V4L2
./run.sh stream serve --device 0 --port 8090

# Vraie caméra IP par RTSP (vérifié sur du matériel réel - voir CHANGELOG.md)
...
**Ce qui fonctionne aujourd'hui :** la validation de config par caméra, la génération de la description du pipeline GStreamer, et la génération de la config de relais MediaMTX (`config.py`, `pipeline.py`, `mediamtx_config.py`), un vrai tampon prouvablement borné et une vraie politique déterministe de reconnexion (`buffer.py`, `reconnect.py`, `stream simulate`), une vraie frontière d'intégration HailoRT (`hailo_runtime.py`) prête pour un vrai module Hailo-8 dès qu'il est branché, et un vrai chemin v0 de capture+diffusion (`mjpeg_server.py`, `stream serve`) qui ouvre un vrai périphérique USB/V4L2 **ou une vraie caméra IP par RTSP** (`source_type: "ip"` dans `config.py`, `cv2.CAP_FFMPEG` dans `mjpeg_server.py`) via OpenCV et diffuse du vrai MJPEG en HTTP - installable sur une CM5 via `provisioning/install_vision_streamer.sh` de `HYDRA-UMC-OS` (une instance systemd par emplacement de caméra assigné par l'administrateur, `systemd/hydra-umc-vision-streamer@.service`) et déjà consommé en direct par le proxy `GET /api/camera/:id/stream` de `HYDRA-UMC-SERVER` et les vues caméra de `HYDRA-UMC-STUDIO`. Le chemin caméra IP est vérifié de bout en bout sur du matériel réel : les 4 vraies caméras IP du réseau local se sont ouvertes et ont diffusé de vraies trames par le chemin réel complet - 83 tests au total, plus un vrai paquet Python installable avec un point d'entrée vérifié et un incrément de version compteur kilométrique intégré au build. Voir [`CHANGELOG.md`](CHANGELOG.md) pour la sortie de build/run capturée.
...
- **[docs/CLI_REFERENCE.md](docs/CLI_REFERENCE.md)** — chaque sous-commande (`config validate`/`gst`/`mediamtx`, `stream simulate`/`serve`, y compris de vrais exemples caméra USB et IP par RTSP), avec une sortie réelle capturée.
