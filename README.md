...
* 📷 **Real v0 - USB and RTSP IP camera capture+serve:** `mjpeg_server.py`'s `stream serve` actually opens a camera and serves real MJPEG over HTTP - a USB/V4L2 device (`--device 0`) or a real RTSP IP camera (`--device rtsp://user:pass@host:port/path`, via OpenCV's `cv2.CAP_FFMPEG` backend, no extra dependency). `config.py`'s `CameraConfig` now carries a real `source_type` (`"usb"` default, or `"ip"` with `host`/`rtsp_port`/`rtsp_path`/`username`/`password` and a real, percent-encoded `rtsp_url()` builder). Verified end to end against real hardware: all 4 real IP cameras on the local network opened and streamed real MJPEG/H.264 frames through this exact path (the 2nd pair needed their own real RTSP path, `profile0`, found on the camera's own config screen - a different path than the first pair's `/11`, not a credential or firmware issue).
* 🔁 **Real v0 - bounded buffering and reconnection:** `buffer.py`'s `FrameBuffer` is a fixed-capacity queue that drops the OLDEST frame (never the newest) once full - the real backpressure policy a live relay needs so a slow consumer can never make this process's memory grow without bound. `reconnect.py`'s `ConnectionTracker` is a real, deterministic exponential-backoff reconnect policy for a dropped camera/relay link. `mjpeg_server.py`'s own real capture path buffers a `Frame` (not just bare bytes) - a real `frame_id`/`session_id`/`capture_ts` identity, with a genuinely new `session_id` on every real reconnect, so two frames from before/after a dropped link are never conflated as one continuous stream. Exposed via `stream simulate` below - fully testable without GStreamer or a physical camera.
...
**Honesty check - what actually runs today:** the config validation, GStreamer pipeline description generation, MediaMTX relay config generation, the real buffer/reconnect policy, and the HailoRT integration boundary (`config.py`, `pipeline.py`, `mediamtx_config.py`, `buffer.py`, `reconnect.py`, `hailo_runtime.py`) are real and tested (86 tests). `stream serve` (`mjpeg_server.py`) is the one path that does open a real camera - a USB/V4L2 device or a real RTSP IP camera - and has been verified against real IP cameras on the local network; it has not yet been verified against a physically-connected USB camera. `/stream` caps concurrent viewers at `--max-clients` (default 5, real 503 beyond that) to protect a CM5's limited Wi-Fi uplink from an unbounded number of simultaneous streams. The full GStreamer/PyGObject pipeline `config gst`/`config mediamtx` only generate the description for, and real Hailo-8 inference, still need that real runtime and a real Hailo-8 module, which this environment doesn't have. See [`CHANGELOG.md`](CHANGELOG.md) for exactly what has shipped so far, and "Current Status & Next Steps" below for what remains open.
...
│       ├── mjpeg_server.py     # Real MJPEG server - actually serves a USB webcam's or RTSP IP camera's picture over HTTP
...
5. **Real test suite** - `python -m pytest tests/ -q` (83 tests covering config (including real IP-camera parsing and `rtsp_url()` percent-encoding), pipeline, MediaMTX generation, the buffer/reconnect policy, the HailoRT integration boundary, the MJPEG capture+serve path, and the CLI).
...
Real example - actually capture+serve a camera as MJPEG over HTTP (the one subcommand that opens a real device):

```bash
# USB/V4L2 device
./run.sh stream serve --device 0 --port 8090

# Real RTSP IP camera (verified against real hardware - see CHANGELOG.md)
...
**What works today:** per-camera config validation, GStreamer pipeline description generation, and MediaMTX relay config generation (`config.py`, `pipeline.py`, `mediamtx_config.py`), a real, provably-bounded frame buffer and a real deterministic reconnect policy (`buffer.py`, `reconnect.py`, `stream simulate`), a real HailoRT integration boundary (`hailo_runtime.py`) ready for a real Hailo-8 module the moment it plugs in, and a real v0 capture+serve path (`mjpeg_server.py`, `stream serve`) that opens a real USB/V4L2 device **or a real RTSP IP camera** (`source_type: "ip"` in `config.py`, `cv2.CAP_FFMPEG` in `mjpeg_server.py`) via OpenCV and serves real MJPEG over HTTP - installable on a CM5 via `HYDRA-UMC-OS`'s own `provisioning/install_vision_streamer.sh` (one systemd instance per admin-assigned camera slot, `systemd/hydra-umc-vision-streamer@.service`) and already consumed live by `HYDRA-UMC-SERVER`'s `GET /api/camera/:id/stream` proxy and `HYDRA-UMC-STUDIO`'s camera views. The IP camera path is verified end to end against real hardware: all 4 real IP cameras on the local network opened and streamed real frames through the complete real path - 83 tests total, plus a real, installable Python package with a verified entry point and an odometer-style version bump wired into the build. See [`CHANGELOG.md`](CHANGELOG.md) for the captured build/run output.
...
* `stream serve` has not yet been verified against a real, physically-connected USB camera - only against a real RTSP IP camera and against `cv2.VideoCapture` mocked at the module boundary (see `tests/test_mjpeg_server.py`).
...
- **[docs/CLI_REFERENCE.md](docs/CLI_REFERENCE.md)** — every subcommand (`config validate`/`gst`/`mediamtx`, `stream simulate`/`serve`, including real USB and RTSP IP camera examples), with real captured output.
