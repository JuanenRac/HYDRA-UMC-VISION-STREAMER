...
5. **Suite de tests real** - `python -m pytest tests/ -q` (86 tests que cubren config (incluido el parseo real de cámaras IP y el percent-encoding de `rtsp_url()`), pipeline, generación de MediaMTX, la política de buffer/reconexión, el límite de integración con HailoRT, la ruta de captura+servido MJPEG, y el CLI).
