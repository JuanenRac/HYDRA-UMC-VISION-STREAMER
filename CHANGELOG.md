...
- **`mjpeg_server.py`'s live capture loop now actually reconnects** (new
  ...
  releasing and reopening it. Past `RECONNECT_AFTER_CONSECUTIVE_FAILURES`
  (10) consecutive failed reads - real, ordinary jitter below that -
  the loop now actually releases and reopens the device on
  `ConnectionTracker`'s own deterministic backoff schedule, and cleanly
  stops itself (a real, honest `GIVEN_UP` outcome in `last_error`) after
  its configured attempt limit rather than retrying forever. New
  end-to-end tests (a fake `cv2.VideoCapture` simulating a real
  disconnect-then-recover, and a real persisted-disconnect) prove both
  the recovery and the give-up path against the real capture loop and
  thread, not just the underlying `ConnectionTracker` in isolation.
